...
public class InventoryModel extends AbstractTableModel {
    ...
    private final String[] columnNamesSQL = new String[] {"INVENTORYID", "ITEM_NAME",
    ...
    public int getValueAtCell(JTable tblMain, int row, int column) {
        String nameOfItem = (String) tblMain.getValueAt(row, column);
        int idOfItem = -1;
        try {
            SQL = "SELECT INVENTORYID from INVENTORY WHERE ITEM_NAME = '" + nameOfItem + "'";
            ResultSet rs = stmt.executeQuery(SQL);
            idOfItem = rs.getInt(1);
        } catch (SQLException err) {
            System.out.println(err.getMessage());
        }
        return idOfItem;
    ...
    public List<Item> getInventory() {
        ...
        try {
            ...
            while(result.next()) {
                Item item = new Item(rs.getInt("INVENTORYID"), rs.getString("ITEM_NAME"),
